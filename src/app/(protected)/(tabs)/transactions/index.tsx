      ...
      >
        ...
        <View className="flex-1 min-w-0 gap-0.5">
          ...
          <View className="flex-row items-center gap-1.5 flex-wrap">
            <AccountAvatar
              avatarUrl={t.account.avatar?.url}
              icon={t.account.icon}
              bgColor={t.account.bgColor}
              iconColor={t.account.color}
              name={t.account.name}
              size={14}
            />
            <Text
            ...
            {t.toAccount && (
              <>
                ...
                />
                <AccountAvatar
                  avatarUrl={t.toAccount.avatar?.url}
                  icon={t.toAccount.icon}
                  bgColor={t.toAccount.bgColor}
                  iconColor={t.toAccount.color}
            ...
            <View className="flex-row gap-1 flex-wrap mt-0.5">
              {t.tags!.slice(0, 3).map((tag) => (
                <View
                  ...
                  style={{
                    backgroundColor:
                      tag.bgColor ?? `${tag.color ?? C.outlineVariant}22`,
                  }}
                >
                  <DynamicIcon
                    name={tag.icon}
                    size={9}
                    color={tag.color ?? C.outlineVariant}
                  />
                  <Text
                    style={{
                      fontSize: 9,
...
function ActiveFiltersBar({
  ...
  if (filter.types.length > 0) {
    chips.push({
      label: `${filter.types.map((t) => t.charAt(0).toUpperCase() + t.slice(1)).join(", ")}`,
      color: "#f59e0b",
  ...
  if (filter.categories.length > 0) {
    chips.push({
      label: `${filter.categories.length} categor${filter.categories.length === 1 ? "y" : "ies"}`,
      color: "#8b5cf6",
      onPress: onOpenFilter,
    });
  }
  if (filter.tags.length > 0) {
    chips.push({
      label: `${filter.tags.length} tag${filter.tags.length === 1 ? "" : "s"}`,
      color: "#10b981",
      onPress: onOpenFilter,
    });
  }
  if (filter.people.length > 0) {
    chips.push({
      label: `${filter.people.length} ${filter.people.length === 1 ? "person" : "people"}`,
      color: "#f59e0b",
      onPress: onOpenFilter,
    });
  }
  if (filter.accounts.length > 0) {
    chips.push({
      label: `${filter.accounts.length} account${filter.accounts.length === 1 ? "" : "s"}`,
      color: "#6366f1",
      onPress: onOpenFilter,
    });
  }
  if (hasSort) {
    ...
    chips.push({
      label: `${SORT_LABELS[sort.field] ?? sort.field} ${dirIcon}`,
      color: C.primary,
      onPress: onOpenSort,
    });
  }
  ...
  return (
    <View className="flex-row items-center px-4 pb-2" style={{ gap: 6 }}>
      <FlatList
        horizontal
        data={chips}
      ...
      >
        ...
        <Text
          style={{ fontSize: 11, color: C.outlineVariant, fontWeight: "600" }}
        >
          Clear
        </Text>
      </TouchableOpacity>
...
export default function HistoryScreen() {
  ...
  const { personId, accountId, dateFrom, dateTo, categoryId, tagId } =
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      if (dateFrom && dateTo)
        setFilter((f) => ({ ...f, dateFrom, dateTo, datePreset: "" }));
      if (categoryId)
        setFilter((f) => ({ ...f, categories: categoryId.split(",") }));
  ...
  const backendSortStr =
    sort.field === "amount" ? toSortString(DEFAULT_SORT) : toSortString(sort);

  const { data, transactions, loading, refetch, fetchMore } =
    useGetTransactions({
      limit: PAGE_SIZE,
      sort: backendSortStr,
      where,
        ...
        updateQuery(
          ...
          return {
            Transactions: {
              ...
              docs: [
                ...(prev.Transactions?.docs ?? []),
                ...fetchMoreResult.Transactions.docs,
              ],
            },
          };
        },
  ...
  if (groupByDate) {
    ...
    for (const [title, group] of groupMap) {
      ...
      for (const t of group)
        listItems.push({ kind: "row", transaction: t, key: t.id });
    }
  } else {
    for (const t of displayTransactions ?? []) {
      listItems.push({
  ...
  const isEmpty =
    !loading && (!displayTransactions || displayTransactions.length === 0);
        ...
        <View className="flex-row items-center justify-between">
          ...
          <View className="flex-row items-center gap-1">
            ...
            <TouchableOpacity
              ...
              style={{
                backgroundColor: showSearch ? `${C.primary}22` : C.surfaceMid,
              }}
            >
              <DynamicIcon
                name={showSearch ? "search-x" : "search"}
                size={17}
                color={showSearch ? C.primary : C.onSurface}
              />
            </TouchableOpacity>
            ...
            <View style={{ position: "relative" }}>
              <TouchableOpacity
                ...
                style={{
                  backgroundColor:
                    filterCount > 0 ? `${C.primary}22` : C.surfaceMid,
                }}
              >
                <DynamicIcon
                  name="sliders-horizontal"
                  size={17}
                  color={filterCount > 0 ? C.primary : C.onSurface}
                />
              </TouchableOpacity>
              {filterCount > 0 && (
                <View
            ...
            <TouchableOpacity
              ...
              className="w-9 h-9 rounded-full items-center justify-center"
              style={{
                backgroundColor: !isDefaultSort(sort)
                  ? `${C.primary}22`
                  : C.surfaceMid,
              }}
            >
              <DynamicIcon
                name="arrow-up-down"
                size={17}
                color={!isDefaultSort(sort) ? C.primary : C.onSurface}
              />
            </TouchableOpacity>
          </View>
        </View>
            ...
            loadingMore ? (
              ...
              <View className="py-3 items-center gap-1">
                <DynamicIcon
                  ...
                  color={C.outlineVariant}
                />
                <Text style={{ fontSize: 11, color: C.outlineVariant }}>
                  Scroll for more
                </Text>
              </View>
            ) : null
