  ...
  const handlePickAvatar = async () => {
    ...
    if (!permission.granted) {
      showAlert({
        ...
        message: "Allow access to your photo library to set an account avatar.",
      });
      return;
    }
    ...
    if (isLocalAvatar && avatarFilename && avatarMime) {
      ...
      try {
        ...
        showAlert({
          title: "Upload failed",
          message:
            "Could not upload the avatar image. The account will be saved without the new image.",
        });
        return;
      }
      setUploading(false);
              ...
              <View className="flex-1">
                ...
                {(account.totalTransactions ?? 0) > 0 && (
                  <Text className="text-[11px] text-on-surface-variant">
                    {account.totalTransactions} transaction
