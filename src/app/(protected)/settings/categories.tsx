    ...
    <View className="flex-1 bg-surface">
      ...
      <View style={{ paddingTop: topPad }} className="px-5 pb-3 bg-surface">
        <View className="flex-row items-center gap-3 mb-4">
          ...
          <TouchableOpacity
            onPress={() =>
              router.push(`/settings/add-category?type=${activeType}`)
            }
            activeOpacity={0.75}
        ...
        {loading && !categories ? (
          ...
          parents.map((parent) => (
            <CategoryCard
              ...
              onEditParent={() =>
                router.push(`/settings/edit-category/${parent.id}`)
              }
              onDeleteParent={() => handleDeleteCategory(parent)}
              onEditChild={(child) =>
                router.push(`/settings/edit-category/${child.id}`)
              }
