...
function TypePill({ amount, color, icon, fmt }: {
  amount: number;
  color: string;
  icon: string;
  fmt: (n: number) => string;
  ...
  return (
    <View className="flex-row items-center gap-0.5 rounded-full px-2 py-0.5" style={{ backgroundColor: `${color}18` }}>
      <DynamicIcon name={icon} size={9} color={color} />
      <Text className="text-[10px] font-bold" style={{ color }}>{fmt(amount)}</Text>
    </View>
  );
}
...
function TagRow({ tag, fmt, onPress }: {
  tag: TagActivity;
  fmt: (n: number) => string;
  onPress: () => void;
}) {
  ...
  return (
    <TouchableOpacity onPress={onPress} activeOpacity={0.7} className="flex-row items-center gap-3">
      <View className="w-9 h-9 rounded-xl items-center justify-center" style={{ backgroundColor: bgColor }}>
        <DynamicIcon name={tag.icon ?? "tag"} size={16} color={iconColor} />
      </View>

      <View className="flex-1 gap-1 min-w-0">
        <Text className="text-[13px] font-bold text-on-surface" numberOfLines={1}>{tag.name}</Text>
        <View className="flex-row flex-wrap gap-1">
          <TypePill amount={tag.expenseAmount} color={EXPENSE_COLOR} icon="trending-down" fmt={fmt} />
          <TypePill amount={tag.incomeAmount} color={INCOME_COLOR} icon="trending-up" fmt={fmt} />
        </View>
      </View>

      <View className="items-end gap-0.5">
        <Text className="text-[13px] font-bold text-on-surface">{fmt(tag.totalAmount)}</Text>
        <Text className="text-[10px] text-on-surface-variant">
          {tag.totalTransactionCount} txn{tag.totalTransactionCount !== 1 ? "s" : ""}
        </Text>
      </View>
    </TouchableOpacity>
  );
...
function TxSheetRow({ tx, fmt }: { tx: TransactionFieldsFragment; fmt: (n: number | null | undefined) => string }) {
  const color = tx.type === "income" ? INCOME_COLOR : tx.type === "expense" ? EXPENSE_COLOR : "#6366f1";
  ...
  return (
    <View className="flex-row items-center gap-3 p-3 rounded-[14px] bg-surface-mid">
      <View className="w-9 h-9 rounded-[10px] items-center justify-center" style={{ backgroundColor: catBgColor }}>
        <DynamicIcon name={catIcon} size={16} color={catColor} />
      </View>
      <View className="flex-1 gap-0.5 min-w-0">
        <Text className="text-[13px] font-semibold text-on-surface" numberOfLines={1}>{tx.title}</Text>
        {accountName ? <Text className="text-[11px] text-on-surface-variant" numberOfLines={1}>{accountName}</Text> : null}
      </View>
      <View className="items-end gap-0.5">
        <Text className="text-[13px] font-bold" style={{ color }}>{sign}{fmt(Number(tx.amount))}</Text>
...
function TagDetailContent({ tag, month, year, onClose }: {
  tag: TagActivity;
  month: number;
  year: number;
  onClose: () => void;
  ...
  const handleViewAll = () => {
    onClose();
    router.push({ pathname: "/(tabs)/history", params: { tagId: tag.id, dateFrom, dateTo } });
  };
  ...
  return (
    <>
      ...
      <View className="flex-row items-center gap-3 px-5 pb-3.5">
        <View className="w-11 h-11 rounded-[13px] items-center justify-center" style={{ backgroundColor: bgColor }}>
          <DynamicIcon name={tag.icon ?? "tag"} size={20} color={iconColor} />
        </View>
        <View className="flex-1">
          <Text className="text-base font-extrabold text-on-surface tracking-tight">{tag.name}</Text>
          <Text className="text-[12px] text-on-surface-variant">{monthName} '{String(year).slice(2)}</Text>
        </View>
      ...
      <View className="flex-row gap-2 px-5 pb-3.5 flex-wrap">
        {tag.expenseAmount > 0 && (
          <View className="flex-row items-center gap-1 rounded-full px-2.5 py-1" style={{ backgroundColor: `${EXPENSE_COLOR}18` }}>
            <DynamicIcon name="trending-down" size={12} color={EXPENSE_COLOR} />
            <Text className="text-[12px] font-bold" style={{ color: EXPENSE_COLOR }}>{fmt(tag.expenseAmount)}</Text>
          </View>
        )}
        {tag.incomeAmount > 0 && (
          <View className="flex-row items-center gap-1 rounded-full px-2.5 py-1" style={{ backgroundColor: `${INCOME_COLOR}18` }}>
            <DynamicIcon name="trending-up" size={12} color={INCOME_COLOR} />
            <Text className="text-[12px] font-bold" style={{ color: INCOME_COLOR }}>{fmt(tag.incomeAmount)}</Text>
          </View>
        )}
      ...
      >
        {loading
          ? [1, 2, 3].map((i) => <Skeleton key={i} width="100%" height={60} radius={14} style={{ marginBottom: 6 }} />)
          : transactions?.length
            ? transactions.map((tx) => <TxSheetRow key={tx.id} tx={tx} fmt={fmt} />)
            : (
              <View className="py-7 items-center">
                <Text className="text-[13px] text-on-surface-variant">No transactions found</Text>
              </View>
            )}
      </ScrollView>
      ...
      <View className="px-4 pt-3" style={{ paddingBottom: Math.max(insets.bottom, 16) + 4 }}>
        ...
        >
          <Text className="text-[14px] font-bold text-primary-bright">
            {tag.totalTransactionCount > 0 ? `View All ${tag.totalTransactionCount} Transactions` : "View in History"}
          </Text>
        </TouchableOpacity>
      </View>
    </>
...
export function SpendingTagsCard() {
  ...
  const tags: TagActivity[] = (data?.data ?? []).filter(
    (t): t is TagActivity => t != null && t.totalAmount > 0,
  ).sort((a, b) => b.totalAmount - a.totalAmount);

  const meta = data?.meta;
  const monthName = meta?.monthName ?? new Date(year, month - 1).toLocaleString("en-US", { month: "long" });
    ...
    <>
      <View className="rounded-2xl p-5 gap-3.5 bg-surface-low">
        ...
        <View className="flex-row items-center justify-between">
          <Text className="text-base font-extrabold tracking-tight text-on-surface">Tag Activity</Text>
          <View className="flex-row items-center gap-1.5">
            ...
            <TouchableOpacity
              onPress={() => router.push({ pathname: "/(tabs)/history", params: { dateFrom, dateTo } })}
              activeOpacity={0.8}
              className="px-2.5 py-1 rounded-full bg-surface-highest"
            >
              <Text className="text-[11px] font-bold text-primary-bright">{monthLabel}</Text>
            </TouchableOpacity>
        ...
        <View className="gap-3.5">
          {showSkeleton
            ...
            : tags.map((tag) => (
                <TagRow key={tag.id} tag={tag} fmt={fmt} onPress={() => setSelected(tag)} />
              ))}
        </View>
      </View>
