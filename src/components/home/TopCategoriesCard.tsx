...
function CategoryCardSkeleton() {
  ...
  return (
    <View className="rounded-2xl overflow-hidden bg-surface-mid">
      ...
      <View className="mx-3.5" style={{ height: 1, backgroundColor: `${C.outlineVariant}44` }} />
...
function ChildRow({
  ...
  return (
    ...
    >
      <View className="w-px self-stretch" style={{ backgroundColor: `${C.outlineVariant}80` }} />
      <View
      ...
      <Text className="flex-1 text-[12px] font-medium text-on-surface-variant" numberOfLines={1}>
        {child.name}
      </Text>
...
function CategoryDetailContent({
  ...
  const handleViewAll = () => {
    onClose();
    router.push({ pathname: "/(tabs)/history", params: { categoryId: category.id, dateFrom, dateTo } });
  };
    ...
    <>
      ...
      <View className="flex-row items-center gap-3 px-5 pb-3.5">
        <View
          ...
          style={{ backgroundColor: bgColor }}
        >
          <DynamicIcon name={category.icon ?? "folder"} size={20} color={iconColor} />
        </View>
        <View className="flex-1">
          <Text className="text-base font-extrabold text-on-surface tracking-tight">{category.name}</Text>
      ...
      <View className="flex-row gap-2 px-5 pb-3.5">
        <View className="flex-row items-center gap-1 rounded-full px-2.5 py-1" style={{ backgroundColor: "#ef444418" }}>
          ...
          <Text className="text-[12px] font-bold" style={{ color: "#ef4444" }}>{fmt(category.amount)}</Text>
        </View>
      ...
      >
        ...
        >
          <Text className="text-[14px] font-bold text-primary-bright">
            {category.transactionCount > 0 ? `View All ${category.transactionCount} Transactions` : "View in History"}
          </Text>
        </TouchableOpacity>
      </View>
    </>
...
function ParentCard({
  ...
  const barPct = totalExpenses > 0 ? Math.min(parent.amount / totalExpenses, 1) : 0;
    ...
    <View className="rounded-2xl overflow-hidden bg-surface-mid">
      ...
      >
        <View
          ...
          style={{ backgroundColor: bgColor }}
        >
          <DynamicIcon name={parent.icon ?? "folder"} size={16} color={iconColor} />
        </View>

        <View className="flex-1 gap-1.5">
          <View className="flex-row items-center justify-between">
            <Text className="text-[13px] font-bold text-on-surface" numberOfLines={1}>
              {parent.name}
            </Text>
            <View className="flex-row items-center gap-1.5">
              <Text className="text-[10px] text-on-surface-variant">{pct}%</Text>
              <Text className="text-[13px] font-bold text-on-surface">{fmt(parent.amount)}</Text>
            </View>
          </View>
          {/* Progress bar */}
          <View className="h-[3px] rounded-full bg-surface-highest">
            <View
              style={{ width: `${barPct * 100}%`, height: "100%", borderRadius: 2, backgroundColor: iconColor }}
            />
          </View>
        </View>
      </TouchableOpacity>
...
export function TopCategoriesCard() {
  ...
  const hasAnyChildren = parents.some((p) => (childrenMap[p.id]?.length ?? 0) > 0);
    ...
    <View className="rounded-2xl p-5 gap-3.5 bg-surface-low">
      ...
      <View className="flex-row items-center justify-between">
        <View className="gap-0.5">
          ...
          {totalExpenses > 0 && (
            <Text className="text-[11px] text-on-surface-variant">Total {fmt(totalExpenses)}</Text>
          )}
        </View>
        <View className="flex-row items-center gap-1.5">
        ...
        {showSkeleton
          ...
          : parents.map((parent) => (
              <ParentCard
                key={parent.id}
                parent={parent}
                children={(childrenMap[parent.id] ?? []).sort((a, b) => b.amount - a.amount)}
      ...
      >
        ...
        />
        <View
          className="absolute bottom-0 left-0 right-0 rounded-t-3xl bg-surface-low"
        >
          {selected && (
