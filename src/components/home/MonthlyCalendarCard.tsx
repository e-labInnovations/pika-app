...
function DayCell({
  ...
  const expStr = dayData && dayData.expenses > 0 ? fmtCell(dayData.expenses) : "";
  ...
  return (
    ...
    >
      ...
      >
        <Text
          className={isToday ? "text-[11px] font-extrabold text-white" : "text-[11px] font-normal text-on-surface"}
        >
          {day}
        </Text>
      </View>
      ...
      {loading ? (
        ...
        <Text className="text-[8px] font-semibold leading-[10px]" style={{ color: "#10b981" }} numberOfLines={1}>
          {incStr}
        </Text>
      ) : expStr ? (
        <Text className="text-[8px] font-semibold leading-[10px]" style={{ color: "#ef4444" }} numberOfLines={1}>
          {expStr}
        </Text>
      ) : isEmpty ? (
        <Text className="text-[9px] leading-[10px]" style={{ color: `${C.outlineVariant}60` }}>—</Text>
      ) : null}
  ...
  return (
    <View className="flex-row items-center gap-3 p-3 rounded-[14px] bg-surface-mid">
      ...
      <View className="flex-1 gap-0.5 min-w-0">
        ...
        {accountName ? (
          <Text className="text-[11px] text-on-surface-variant" numberOfLines={1}>{accountName}</Text>
        ) : null}
      </View>
  ...
  return (
    <>
      ...
      <View className="flex-row items-center justify-between px-5 pb-3">
        <Text className="text-base font-extrabold text-on-surface tracking-tight">{displayDate}</Text>
      ...
      {dayData && (dayData.income > 0 || dayData.expenses > 0) && (
        <View className="flex-row gap-2 px-5 pb-3.5">
          {dayData.income > 0 && (
            <View className="flex-row items-center gap-1 rounded-full px-2.5 py-1" style={{ backgroundColor: "#10b98118" }}>
              <DynamicIcon name="trending-up" size={12} color="#10b981" />
              <Text className="text-[12px] font-bold" style={{ color: "#10b981" }}>+{fmt(dayData.income)}</Text>
            </View>
          )}
          {dayData.expenses > 0 && (
            <View className="flex-row items-center gap-1 rounded-full px-2.5 py-1" style={{ backgroundColor: "#ef444418" }}>
              <DynamicIcon name="trending-down" size={12} color="#ef4444" />
              <Text className="text-[12px] font-bold" style={{ color: "#ef4444" }}>-{fmt(dayData.expenses)}</Text>
            </View>
          )}
          {dayData.transfers > 0 && (
            <View className="flex-row items-center gap-1 rounded-full px-2.5 py-1" style={{ backgroundColor: "#6366f118" }}>
              <DynamicIcon name="arrow-left-right" size={12} color="#6366f1" />
              <Text className="text-[12px] font-bold" style={{ color: "#6366f1" }}>{fmt(dayData.transfers)}</Text>
            </View>
          )}
        </View>
      )}
          ...
          : transactions?.length
            ...
            : (
              <View className="py-7 items-center">
                <Text className="text-[13px] text-on-surface-variant">No transactions on this day</Text>
              </View>
            )}
      </ScrollView>
      ...
      <View className="px-4 pt-3" style={{ paddingBottom: Math.max(insets.bottom, 16) + 4 }}>
        <TouchableOpacity onPress={handleViewAll} activeOpacity={0.8} className="p-4 rounded-2xl items-center bg-surface-mid">
          <Text className="text-[14px] font-bold text-primary-bright">
            {totalCount > 0 ? `View All ${totalCount} Transactions` : "View in History"}
          </Text>
        </TouchableOpacity>
      </View>
    </>
...
function CalendarSkeleton() {
  const cells = buildCalendarCells(new Date().getFullYear(), new Date().getMonth() + 1);
  return (
    <View>
...
export function MonthlyCalendarCard() {
  ...
  data?.data?.forEach((d) => { if (d) dayMap.set(d.date.slice(0, 10), d); });

  const cells = buildCalendarCells(year, month);
  const isCurrentMonth = today.getFullYear() === year && today.getMonth() + 1 === month;

  const monthLabel =
    (data?.meta?.monthName ?? new Date(year, month - 1).toLocaleString("en-US", { month: "long" })) +
    ` '${String(year).slice(2)}`;
    ...
    <>
      <View className="rounded-2xl p-5 gap-3.5 bg-surface-low">
        ...
        <View className="flex-row items-center justify-between">
          <Text className="text-base font-extrabold tracking-tight text-on-surface">Monthly</Text>
        ...
        <View className="flex-row">
          {WEEK_DAYS.map((d) => (
            <View key={d} className="flex-1 items-center">
              <Text className="text-[9px] font-bold tracking-[0.3px] text-on-surface-variant">{d}</Text>
            </View>
          ))}
        </View>
        ...
        {/* Footer divider */}
        <View className="h-px" style={{ backgroundColor: `${C.outlineVariant}33` }} />
        ...
        {showSkeleton ? (
          <View className="flex-row items-center">
            {[0, 1, 2].map((i) => (
              <React.Fragment key={i}>
                ...
                {i < 2 && <View className="w-px h-4" style={{ backgroundColor: `${C.outlineVariant}33` }} />}
              </React.Fragment>
            ))}
          </View>
        ) : meta ? (
          <View className="flex-row items-center">
            <View className="flex-1 flex-row items-center justify-center gap-1">
              <DynamicIcon name="trending-up" size={12} color="#10b981" />
              <Text className="text-[11px] font-bold" style={{ color: "#10b981" }}>{fmt(meta.totalIncome)}</Text>
            </View>
            <View className="w-px h-4" style={{ backgroundColor: `${C.outlineVariant}33` }} />
            <View className="flex-1 flex-row items-center justify-center gap-1">
              <DynamicIcon name="trending-down" size={12} color="#ef4444" />
              <Text className="text-[11px] font-bold" style={{ color: "#ef4444" }}>{fmt(meta.totalExpenses)}</Text>
            </View>
            <View className="w-px h-4" style={{ backgroundColor: `${C.outlineVariant}33` }} />
            <View className="flex-1 flex-row items-center justify-center gap-1">
              <DynamicIcon name="wallet" size={12} color={balancePositive ? "#10b981" : "#ef4444"} />
              <Text className="text-[11px] font-bold" style={{ color: balancePositive ? "#10b981" : "#ef4444" }}>
                {fmt(meta.balance)}
              </Text>
            </View>
          </View>
